...
@with_config_key('schema', default="public")
...
class PostgresOutput(Output):
    ...
    def connection(self):
        if not self._connection:
            self._connection = psycopg.connect(host=self._host, port=self._port,
    ...
    def ingest(self, cols: Collection[str], rows: Iterable[Collection[Any]]) -> int:
        fix = uuid.uuid1().hex
        batch_tbl = f'"batch_{fix}"'
        cdiff_tbl = f'"cdiff_{fix}"'
        udiff_tbl = f'"udiff_{fix}"'
        known_tbl = f'"{self._schema}"."{self._table}"'
        ...
        def _same_as(t):
            return ' AND '.join([f'{known_tbl}."{c}"={t}."{c}"' for c in key_cols])

        __create_batch = (
            f'CREATE TEMP TABLE {batch_tbl} ON COMMIT DROP AS '
            f'SELECT {_just_cols} FROM {known_tbl} LIMIT 0'
        )
        __insert_into_batch = (
        ...
        __create_cdiff = (
            ...
            f'WHERE NOT EXISTS (SELECT 1 FROM {known_tbl} WHERE {_same_as(batch_tbl)}) '
        )
        __create_udiff = (
        ...
        __insert_from_cdiff = (
            f'INSERT INTO {known_tbl} ({_just_cols}) SELECT {_just_cols} FROM {cdiff_tbl}'
        )
        __update_from_udiff = (
            f'UPDATE {known_tbl} SET {_set_from(udiff_tbl)} FROM {udiff_tbl} WHERE {_same_as(udiff_tbl)}'
        )

        __insert_from_batch = (
            f'INSERT INTO {known_tbl} ({_just_cols}) SELECT {_just_cols} FROM {batch_tbl}'
        )
